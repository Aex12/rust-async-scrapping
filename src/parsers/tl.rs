...
pub fn parse(input: &str, selectors: Selectors) -> Result<Product> {
    ...
    let get = |selector: &str| -> Result<&tl::Node> {
        dom
            ...
            .context("Failed to run query selector")?
            .next()
            .context("Failed to find an element matching query selector")?
            .get(parser)
            .context("Parser failed to get element from query selector")
    };

    let get_text =
        |selector: &str| -> Result<String> { Ok(get(selector)?.inner_text(parser).to_string()) };

    let get_attribute = |selector: &str, attr: &str| -> Result<String> {
        Ok(get(selector)?
            .as_tag()
            .context("Couldn't convert to tag")?
            .attributes()
            .get(attr)
            .flatten()
            .context("Couldnt get attribute")?
            .try_as_utf8_str()
            .context("Couldn't convert to utf8 str")?
            .to_string())
    };
